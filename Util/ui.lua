...
--Folder Maker
if not isfile("CrumblewareV7") then
    makefolder("CrumblewareV7")
end

if not isfile("CrumblewareV7/" .. Game) then
    makefolder("CrumblewareV7/" .. Game)
end

if not isfile("CrumblewareV7/".. Game .. "/Configs") then
    makefolder("CrumblewareV7/".. Game .. "/Configs")
end

if not isfile("CrumblewareV7/Luas") then
    makefolder("CrumblewareV7/Luas")
end

if not isfile("CrumblewareV7/Themes") then
    makefolder("CrumblewareV7/Themes")
end
...
menu.bg.Position     = UDim2.new(0.5,-menu.bg.Size.X.Offset,0.5,-menu.bg.Size.Y.Offset/2)
local foldercool = Instance.new("Folder")
...
Text.TextSize = 14
local User = LRM_LinkedDiscordID or "Unknown"
local FPS = 0
local Ping = 0
Text.Text = "    Crumbleware  <font color='#c375ae'>V7</font>  |  User:  <font color='#c375ae'>"..User.."</font>  |  Fps:  <font color='#c375ae'>"..FPS.."</font>  |  Ping:  <font color='#c375ae'>"..Ping.."</font>  |  Time:  <font color='#c375ae'>0:0:0</font>    "
...
spawn(function()
    while wait(1) do
        ...
        local Time = math.floor(game:GetService("Lighting").ClockTime)
        User = _G.LRM_LinkedDiscordID or "Unknown"
        Text.Text = "    Crumbleware  <font color='#c375ae'>V7</font>  |  User:  <font color='#c375ae'>"..User.."</font>  |  Fps:  <font color='#c375ae'>"..FPS.."</font>  |  Ping:  <font color='#c375ae'>"..Ping.."</font>  |  Time:  <font color='#c375ae'>"..Time.."</font>    "
        FPS = 0
    end
end)
...
function cursorupdate()
    ...
    Cursor.Color = Color3.fromRGB(240, 142, 214);
...
--args.multiselect and "..." or ""
            ...
            gradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(105, 105, 105)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(121, 121, 121))} -- ngl pasted
            gradient.Rotation = 90
            gradient.Name = "gradient"
            gradient.Parent = main
            ...
            holder.BorderColor3 = Color3.fromRGB(60, 60, 60) -- kinda black
            ...
            function refresh(tbl)
                ...
                updateValue(table.find(library.options[args.flag].values,library.flags[args.flag]) and library.flags[args.flag] or library.options[args.flag].values[1]) -- nigger
            end
        ...
        function group:addConfigbox(args)
            ...
            holder.TopImage = "rbxasset://textures/ui/Scroll/scroll-middle.png" -- cock
        ...
        function group:addColorpicker(args)
            ...
            text.TextColor3 = Color3.fromRGB(244, 244, 244)
            text.TextSize = 13.000
            text.ZIndex = args.ontop and topStuff or jigCount
            text.TextStrokeTransparency = 0.000
            text.TextXAlignment = Enum.TextXAlignment.Left
...
function library:createConfig()
    ...
    if name == "" then return library:notify("Crumbleware V7","No Config Name",5) end
    local jig = {}
    ...
    writefile("CrumblewareV7/".. Game .. "/Configs/"..name..".cfg",game:GetService("HttpService"):JSONEncode(jig))
    library:notify("Crumbleware V7","Succesfully created config "..name.."!",5)
    library:refreshConfigs()
end

function library:saveConfig()
    local name = library.flags["selected_config"] or ""
    if name == "" then
        library:notify("Crumbleware V7","No config selected",5)
        return
    end
    local jig = {}
    for i,v in next, library.flags do
    ...
    library:notify("Crumbleware V7","Succesfully updated config "..name.."!",5)
    library:refreshConfigs()
end
...
function library:loadConfig()
    ...
    if not isfile(name) then
        library:notify("Crumbleware V7","Config file not found!",5)
        return
    end
    ...
    library:notify("Crumbleware V7","Succesfully loaded config "..name.."!",5)
end

function library:refreshConfigs()
    local tbl = {}
    for i,v in next, listfiles("CrumblewareV7/".. Game .. "/Configs") do
        table.insert(tbl,v)
    end
